...
function get() {
    API({
        url: `/data_proxy/search/content=${getQueryContent('content')}&type=${getQueryContent('type')}`,
        method: 'get'
    }).then((e) => {
...
function gotoResult() {
    window.open(`../pages/search.html?content=${content.value}&type=${searchOptionVal.value}`, "_self")
}
